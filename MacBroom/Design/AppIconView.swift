...
/// Stylized retro app icon: cream rounded square, rainbow-striped apple with leaf,
/// and a diagonal broom in the bottom-right with bits being swept.
/// Designed to render correctly at any size — all measurements are proportional.
struct AppIconView: View {
    /// Pixel size of the icon (square). All internal sizes scale from this.
    let size: CGFloat

    init(size: CGFloat = 1024) { self.size = size }

    var body: some View {
        let cornerRadius = size * 0.225        // macOS Big Sur curve
        let appleWidth   = size * 0.58
        let appleHeight  = size * 0.65
        let appleCenterY = size * 0.06         // shift apple slightly down
    ...
    private func background(cornerRadius: CGFloat) -> some View {
        RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
            ...
            .overlay(
                // Subtle inner highlight at top — gives a soft 3D feel
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .strokeBorder(
                        LinearGradient(
    ...
    private var rainbowStripes: LinearGradient {
        // Six retro Apple-style bands, hard transitions (no blending).
    ...
    private var leafShape: some View {
        LeafPath()
            ...
            .overlay(
                LeafPath()
                    .stroke(Color.black.opacity(0.20), lineWidth: size * 0.005)
            )
    }
...
/// Apple silhouette with a bite on the right side. Bite + colored stripes + custom leaf
/// keep this distinct from Apple's trademarked logo.
struct AppleSilhouette: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        p.move(to: cleft)
        // Right lobe — go up and over to the top of the bite
        p.addCurve(
            to: CGPoint(x: w * 1.00, y: h * 0.27),
            control1: CGPoint(x: w * 0.70, y: -h * 0.04),
            control2: CGPoint(x: w * 1.02, y: h * 0.10)
        )
        // Bite — clean semicircle indent on the right edge, faithful to the classic Apple mark
        p.addArc(
            center: CGPoint(x: w * 1.00, y: h * 0.43),
            radius: h * 0.16,
...
struct LeafPath: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let w = rect.width
        let h = rect.height
        // Symmetric vertical leaf: tip at top center, base at bottom center,
        // widest around 45% of height.
...
/// Broom drawn from scratch — vertical handle, golden straw bristles flaring at the bottom.
...
struct BristleFan: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let w = rect.width
        let h = rect.height
        // Symmetric fan: narrow top, splayed bottom.
